...
def post_process_boxes(boxes, boundary, conf=0.6):
    ...
    for i, box in enumerate(boxes):
        if (
            box[-1] != 5
            and box[-1] != 3
            and box[4].item() >= conf * 100
            and (box[2] - box[0]) * (box[3] - box[1]) > MIN_BOX_SIZE
            ...
            if box[0] > XDIM * (DIST_FROM_EDGE) and box[2] < XDIM * (
                1 - DIST_FROM_EDGE
            ):  # Removes Boxes too close to edge which are likely to be misdetected
                if box[1] > boundary:
...
def get_coarse_line_positions(boxes):
    ...
    for i in range(len(boxes[:-1])):
        ...
        position_delta = (next_box[0] + (next_box[2] - next_box[0]) / 2) - (
            curr_box[0] + (curr_box[2] - curr_box[0]) / 2
        )
                ...
                points_out = t.tensor(
                    [
                        [
                            iX + dX * j,
                            iY + dY * j,
                            current_val + j * degree_gap / steps,
                            point[3],
                        ]
                    ]
                )
...
def label_fine_points(points):
    ...
    for i, point in enumerate(points[:-1]):
        ...
        for j in range(int(steps.item())):
            if points_out is None:
                points_out = t.tensor(
                    [[iX + dX * j, iY + dY * j, current_val + j * degree_gap / steps]]
                )
            else:
                points_out = t.cat(
                    (
...
def infer_measure(
    ...
    fine_boxes, coarse_boxes, numeral_positions = post_process_boxes(
        dets, image.shape[0] / 2, CONF_THRESH
    )
