...
def post_process_boxes(boxes,boundary,conf=0.6):
    ...
    for i,box in enumerate(boxes):
        if box[-1] not in [3,5] and box[4].item() >= conf and box_areas[i] > MIN_BOX_SIZE: # Remove A and B Labels, Detections Below Confidence Threshold and Small Bounding Boxes
            if box[0] > XDIM*(DIST_FROM_EDGE) and box[2] < XDIM*(1-DIST_FROM_EDGE): # Removes Boxes too close to edge which are likely to be missdetected
                if box[1] > boundary:
            ...
            if j == 0:
                ...
                if i < len(boxes) - 1:
                    ...
                    if line_labels is None:
                        ...
                        line_labels = t.cat((line_labels,t.tensor([second - 10])),axis=0)
